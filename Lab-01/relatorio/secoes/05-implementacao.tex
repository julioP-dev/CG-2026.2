...
A captura da posição do mouse foi implementada na classe \texttt{JanelaPrincipal} por meio de um \textit{listener} de eventos. O evento de movimentação fornece as coordenadas do cursor em relação ao display. Esses valores são utilizados para criar um objeto do tipo \texttt{Ponto}, que logo após é encaminhado aos métodos da classe \texttt{Transformacao}.

A atualização das informações da interface e do pixel ativo ocorre dentro do método responsável por processar essas coordenadas. Dessa forma, a movimentação do cursor provoca a atualização contínua dos valores apresentados e da posição do pixel.
...
A interface gráfica foi construída utilizando componentes do Swing. A classe \texttt{JanelaPrincipal} organiza o display, as informações das coordenadas e os campos destinados à configuração da janela de mundo.
...
A integração entre as classes ocorre principalmente na \texttt{JanelaPrincipal}. Quando uma nova posição do mouse é recebida, a classe cria o ponto inicial e utiliza os métodos da \texttt{Transformacao} para obter os valores correspondentes aos diferentes sistemas de coordenadas. Depois das transformações necessárias, a coordenada final é arredondada com \texttt{Math.round()} e enviada para o método \texttt{drawPixel()} da classe \texttt{Display}.
