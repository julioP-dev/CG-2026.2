...
\paragraph{Posição do mouse.} A primeira entrada é a posição do mouse sobre o display. O display possui resolução de $800 \times 600$ pixels, sendo que a origem das coordenadas começa no canto superior esquerdo.

A posição do mouse é rastreada continuamente: as conversões são realizadas automaticamente sempre que o mouse se move sobre o display, quando um botão é pressionado e enquanto o mouse é arrastado com o botão pressionado. A cada um desses eventos, a aplicação obtém as coordenadas $x$ e $y$ fornecidas pelo evento e cria um objeto do tipo \texttt{Ponto} com tais valores. A coordenada obtida, $P_{inp} = (x_{inp}, y_{inp})$, é utilizada como entrada para as transformações. Os eventos são capturados por um \textit{listener} associado ao display, de modo que cada nova posição substitui o pixel ativo anterior e atualiza os valores exibidos, sem a necessidade de clicar.
...
A troca do sistema altera apenas quais funções de transformação serão utilizadas nas próximas interações; ela não dispara, por si só, uma nova ativação de pixel. Dessa forma, o pixel não é deslocado para um ponto que o usuário não solicitou, e a próxima movimentação do mouse ou consulta já utiliza o sistema escolhido. Independentemente da seleção, a interface sempre exibe o ponto nos dois sistemas NDC, o que permite compará-los.
...
\label{subsec:fora-limites}
Uma coordenada pode ficar fora dos limites do display em duas situações. A primeira ocorre durante o arraste: ao manter o botão pressionado e levar o mouse para fora do display, o Swing continua enviando eventos, agora com coordenadas negativas ou maiores que $799$ (em $x$) e $599$ (em $y$). A segunda ocorre na consulta de um ponto do mundo situado fora da janela de mundo, que, pelas transformações, é levado a uma posição fora do display.

Nesses casos, as transformações produziriam valores fora dos intervalos válidos, como NDC negativos ou maiores que $1$ e coordenadas de dispositivo negativas, que não correspondem a nenhum pixel. Por isso, a aplicação verifica, antes de exibir as conversões, se a coordenada de dispositivo está dentro do display. Quando não está, o pixel ativo é apagado, os valores de dispositivo, NDC e mundo são substituídos pela indicação ``fora dos limites'' e uma mensagem de alerta é exibida. Para o mouse, o alerta é mostrado em uma linha da própria interface, em vez de uma caixa de diálogo, que seria aberta repetidamente durante o arraste. Para a consulta de ponto do mundo, além dessa linha, é exibida uma caixa de diálogo de aviso.
...
De forma resumida, o funcionamento da solução pode ser representado por dois fluxos. O primeiro corresponde à posição do mouse:
\[
...
\]
Em ambos os fluxos, a etapa NDC utiliza o sistema escolhido no seletor (Subseção~\ref{subsec:selecao-ndc}), e coordenadas fora do display são interrompidas antes da ativação do pixel, gerando um alerta (Subseção~\ref{subsec:fora-limites}).
